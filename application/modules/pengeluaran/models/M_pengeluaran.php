...
class M_pengeluaran extends CI_Model
{
    ...
    public function get_data($search,$from_page,$per_page){
      ...
      $where = ($search == '') ? '' : $where;
      $query = $this->db->query("SELECT id_pengeluaran, DATE_FORMAT(tanggal, '%d/%m/%Y') tanggal,
      keterangan 'Keterangan', kat.nama 'Kategori',format(jumlah,0) Jumlah
      FROM pengeluaran left join kategori_pengeluaran kat using(id_kategori_pengeluaran) $where LIMIT $from_page,$per_page");
